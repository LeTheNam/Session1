...
# LAY DANH SACH THU MUC VA NEWSGROUP
...
# THU THAP DATA
with open('./DS_LAB/Session_1/TF_IDF/20news_bydate/stop_words.txt') as f:
...
def generate_vocabulary(data_path):   
    ...
    # Sap xep theo -idf tu lon toi be
    words_idfs.sort(key= takeSecond, reverse= True) 
    print('Vocabulary size: {}'.format(len(words_idfs)))
    with open('./DS_LAB/Session_1/TF_IDF/20news_bydate/words_idfs.txt', 'w') as f:
        f.write('\n'.join(word + '<fff>' + str(idf) for word, idf in words_idfs))

# TINH TF-IDF
def get_tf_idf(data_path, save_data_path):
    with open('./DS_LAB/Session_1/TF_IDF/20news_bydate/words_idfs.txt') as f:
...
def main():
    train_dir, test_dir, list_newsgroups = gather_20newsgroups_data('./DS_LAB/Session_1/TF_IDF/20news_bydate/')
    ...
    with open('./DS_LAB/Session_1/TF_IDF/20news_bydate/20news_train_processed.txt', 'w') as f:
        f.write('\n'.join(train_data))

    with open('./DS_LAB/Session_1/TF_IDF/20news_bydate/20news_test_processed.txt', 'w') as f:
        f.write('\n'.join(test_data))

    with open('./DS_LAB/Session_1/TF_IDF/20news_bydate/20news_full_processed.txt', 'w') as f:
        f.write('\n'.join(full_data))
    
    generate_vocabulary('./DS_LAB/Session_1/TF_IDF/20news_bydate/20news_full_processed.txt')

    get_tf_idf('./DS_LAB/Session_1/TF_IDF/20news_bydate/20news_train_processed.txt', './DS_LAB/Session_1/TF_IDF/20news_bydate/train_tf_idf.txt')

    get_tf_idf('./DS_LAB/Session_1/TF_IDF/20news_bydate/20news_test_processed.txt', './DS_LAB/Session_1/TF_IDF/20news_bydate/test_tf_idf.txt')
